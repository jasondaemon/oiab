...
Provider credits and icon attributions are maintained in `docs/ATTRIBUTIONS.md` and Central Settings -> Data & Attributions.
...
| Historic Places | cached GeoJSON | refreshes National Park Service NRHP point and polygon layers; see `docs/overlays/historic-places.md` |
| OpenCaching | cached GeoJSON | refreshes OpenCaching.us OKAPI bbox data with a consumer key, or imports local GPX files; see `docs/overlays/opencaching.md` |
| Campflare Campgrounds | cached GeoJSON | refresh with a user-provided Campflare API key; OIAB downloads the latest provider dumps and caches campground points |
| Campflare Campsites | cached GeoJSON | generated by the same Campflare refresh; contains individual campsite points and can be large |
| Campflare Land POIs | cached GeoJSON | generated by the same Campflare refresh from Campflare POI data |
| Campflare Public Lands | cached GeoJSON | generated by the same Campflare refresh where Campflare land geometry/points are available |
| Campflare Notices | cached GeoJSON | generated by the same Campflare refresh from campground notices/alerts in the dump |
...
/data/oiab/maps/overlays/campflare/geojson/campgrounds.geojson
/data/oiab/maps/overlays/campflare/geojson/campsites.geojson
/data/oiab/maps/overlays/campflare/geojson/land-pois.geojson
/data/oiab/maps/overlays/campflare/geojson/public-lands.geojson
/data/oiab/maps/overlays/campflare/geojson/notices.geojson
/data/oiab/maps/overlays/drought/usdm-latest.geojson
/data/oiab/maps/overlays/darksky/tiles/{z}/{x}/{y}.png
```

Source/cache directories under `padus/source`, `water/source`, `public-lands/source`, `darksky/source`, `campflare/raw`, `contours/regions`, and `weather/forecast-cache` are excluded from local overlay auto-registration so generated working files do not show up as duplicate user overlays.
...
| Campflare overlays | Paste a Campflare API key once on any Campflare overlay. Download/Update refreshes all Campflare overlays from the latest signed dump manifest. | <https://campflare.com/platform> |
...
Campflare is integrated as one keyed provider with multiple generated overlays:

- Campflare Campgrounds
- Campflare Campsites
- Campflare Land POIs
- Campflare Public Lands
- Campflare Notices

Set `OIAB_CAMPFLARE_API_KEY` in the environment or paste the key into Settings → Maps → Overlays → a Campflare overlay → Details & Source. The key is sent as the `Authorization` header to:

```text
https://api.campflare.com/v2/dumps/latest
```

OIAB downloads the signed dump files into:

```text
/data/oiab/maps/overlays/campflare/raw
```

Then it generates map-ready GeoJSON into:

```text
/data/oiab/maps/overlays/campflare/geojson
```

The provider refresh is intentionally shared: pressing Download/Update on any Campflare overlay refreshes all Campflare-generated overlays. The UI attribution should include:

```text
Campground data powered by Campflare
```

Campflare data is not committed to the repo. Generated cache files are user/provider data and belong on durable storage.
